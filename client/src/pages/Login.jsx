...
import { FaArrowLeft, FaArrowRight, FaCheck, FaEye, FaEyeSlash, FaLock, FaUtensils } from "react-icons/fa";
...
const Login = () => {
  ...
  return (
    <main className="relative isolate min-h-[calc(100vh-5rem)] overflow-hidden bg-[#fff8f1] px-4 py-8 sm:px-6 lg:px-8 lg:py-14">
      <div className="pointer-events-none absolute -left-24 top-6 -z-10 h-72 w-72 rounded-full bg-orange-200/45 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-20 right-0 -z-10 h-80 w-80 rounded-full bg-rose-200/40 blur-3xl" />
      <div className="mx-auto grid max-w-6xl overflow-hidden rounded-[2rem] border border-white/80 bg-white shadow-[0_30px_90px_rgba(113,52,18,0.16)] lg:grid-cols-[1.05fr_0.95fr]">
        <section className="relative hidden min-h-[620px] overflow-hidden p-10 text-white lg:flex lg:flex-col lg:justify-between lg:p-14">
          ...
          <div className="relative z-10 flex items-center gap-2 text-sm text-white/70"><span className="grid h-5 w-5 place-items-center rounded-full bg-white/15 text-[9px]"><FaCheck /></span> Made for everyday cravings, big and small.</div>
        </section>

        <section className="p-6 sm:p-10 lg:p-14">
          <div className="mb-9 flex items-center justify-between lg:hidden"><div><p className="text-2xl font-black text-orange-700">Cravings</p><p className="mt-1 text-sm text-slate-500">Good food feels like home.</p></div><Link to="/" className="rounded-full bg-orange-50 p-2.5 text-orange-700 transition hover:bg-orange-100" aria-label="Back to home"><FaArrowLeft size={13} /></Link></div>
          <div className="mb-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input id="email" type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="you@example.com" autoComplete="email" aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? "email-error" : undefined} className={`w-full rounded-2xl border bg-slate-50 px-4 py-3.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-orange-400 focus:bg-white focus:ring-4 focus:ring-orange-100 ${errors.email ? "border-red-400" : "border-slate-200"}`} />
              {errors.email && <p id="email-error" className="mt-1.5 text-xs font-semibold text-red-500">{errors.email}</p>}
            </div>
            <div>
              ...
              <div className="relative"><input id="password" type={showPassword ? "text" : "password"} name="password" value={formData.password} onChange={handleInputChange} placeholder="Enter your password" autoComplete="current-password" aria-invalid={Boolean(errors.password)} aria-describedby={errors.password ? "password-error" : undefined} className={`w-full rounded-2xl border bg-slate-50 px-4 py-3.5 pr-12 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-orange-400 focus:bg-white focus:ring-4 focus:ring-orange-100 ${errors.password ? "border-red-400" : "border-slate-200"}`} /><button type="button" onClick={() => setShowPassword((previous) => !previous)} className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full p-1 text-slate-400 transition hover:bg-orange-50 hover:text-orange-600 focus:outline-none focus:ring-2 focus:ring-orange-200" aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <FaEyeSlash /> : <FaEye />}</button></div>
              {errors.password && <p id="password-error" className="mt-1.5 text-xs font-semibold text-red-500">{errors.password}</p>}
            </div>
